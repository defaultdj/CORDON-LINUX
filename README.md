...
Интерфейс подстраивается под экран: на разрешениях вроде 1024×768 окно занимает не больше доступной
области, включается компактный режим (кнопки-меню вместо подписей, уменьшенные отступы), а длинные
диалоги вроде «Настройки профиля» прокручиваются, оставляя кнопки внизу. Размер можно проверить на
другом разрешении без смены монитора: `CORDON_SCREEN=1024x768 cordon-gui`.
...
| `FATAL ERROR` … `No shaders found for OpenGL` / `Can't setup renderer` | в профиль не попали `gamedata/shaders` — обычное дело для распакованных сборок: подключите каталог данных движка (`cordon edit "<профиль>" --engine-data /usr/share/openxray`) и пересоберите профиль (`cordon prepare "<профиль>" --force`). Лаунчер делает это сам, если находит шейдеры в системном каталоге движка |
